...
@section('content')
        ...
        <div class="layout-px-spacing">
            ...
            <form id="form-upload-soal" enctype="multipart/form-data">
                <div id="soal-container">
                    @foreach ($ujian->detailVisual as $index => $soal)
                        ...
                        <div class="card soal-item mb-4" data-soal-id="{{ $index + 1 }}">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    <label for="jawaban_benar_1_{{ $index + 1 }}">Pilihan Benar 1</label>
                                    <select class="form-control" name="jawaban_benar_1_{{ $index + 1 }}"
                                        id="jawaban_benar_1_{{ $index + 1 }}" required>
                                        <option value="">Pilih </option>
                                        @foreach (range('A', 'F') as $option)
                                            <option value="{{ $option }}"
                                                {{ $soal->jawaban_1 == $option ? 'selected' : '' }}>Pilihan
                                                {{ $option }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="jawaban_benar_2_{{ $index + 1 }}">Pilihan Benar 2</label>
                                    <select class="form-control" name="jawaban_benar_2_{{ $index + 1 }}"
                                        id="jawaban_benar_2_{{ $index + 1 }}" required>
                                        <option value="">Pilih </option>
                                        @foreach (range('A', 'F') as $option)
                                            <option value="{{ $option }}"
                                                {{ $soal->jawaban_2 == $option ? 'selected' : '' }}>Pilihan
                                                {{ $option }}</option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    <script>
           document.addEventListener("DOMContentLoaded", function() {
            let soalCount = {{ json_encode(count($ujian->detailVisual )) }};

            function generateSoal() {
                soalCount++;
                const soalHtml = `
                <input type="hidden" name="detail_ujian_id_${soalCount}"
                            id="detail_ujian_id_${soalCount}" value=" ">
                    <div class="card soal-item mb-4" data-soal-id="${soalCount}">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                <label for="jawaban_benar_1_${soalCount}">Pilihan Benar 1</label>
                                <select class="form-control" name="jawaban_benar_1_${soalCount}" id="jawaban_benar_${soalCount}" required>
                                    <option value="">Pilih</option>
                                    ${Array.from({ length: 5 }, (_, i) => `<option value="${String.fromCharCode(65 + i)}">Pilihan ${String.fromCharCode(65 + i)}</option>`).join('')}
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="jawaban_benar_2_${soalCount}">Pilihan Benar 2</label>
                                <select class="form-control" name="jawaban_benar_2_${soalCount}" id="jawaban_benar_2_${soalCount}" required>
                                    <option value="">Pilih</option>
                                    ${Array.from({ length: 5 }, (_, i) => `<option value="${String.fromCharCode(65 + i)}">Pilihan ${String.fromCharCode(65 + i)}</option>`).join('')}
                                </select>
                            </div>
        ...
        document.getElementById("submit-all").addEventListener("click", async function() {
            ...
            for (const soal of soalItems) {
                ...
                const jsonData = {
                    ...
                    jam: jam, 
                    menit: menit, 
                    jawaban_benar_1: null,
                    jawaban_benar_2: null,
                    detailUjianId: null,
                ...
                // Set correct answer
                const correctAnswer1 = soal.querySelector(`[name="jawaban_benar_1_${soalId}"]`).value;
                jsonData.jawaban_benar_1 = correctAnswer1;
                const correctAnswer2 = soal.querySelector(`[name="jawaban_benar_2_${soalId}"]`).value;
                jsonData.jawaban_benar_2 = correctAnswer2;
                ...
                try {
                    const response = await fetch("{{ url('/guru/update_ujian_visual/' . $ujian->id) }}", {
                        method: "PUT",
                        headers: {
