...
class PerbaikanKodeSekalaSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        foreach ($kuisoner as $key => $value) {
            $ujian = Ujian::where('ujian.kode', 'part5_2')->where('guru_id', 1)
                ->join('detail_kuisoner', 'detail_kuisoner.kode', '=', 'ujian.kode')
                ->join('detail_kuisoner_sekalas', 'detail_kuisoner.id', '=', 'detail_kuisoner_sekalas.detail_kuisoner_id')
                ->where('detail_kuisoner.soal', $value['val0'])
                ->select('detail_kuisoner_sekalas.*')
                ->first();

            $sekala = Sekala::where('sekala', $value['val1'])->first();
            DetailKuisonerSekala::whereId($ujian->id)->update([ 
                'kode_sekala' => $sekala->kode,
            ]);
        }
    }
}
